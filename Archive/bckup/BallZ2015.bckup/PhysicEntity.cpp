...
void EntityForceAndTorqueCallback(const NewtonBody* body, dFloat timestep, int threadIndex)
...
void EntityTransformationCallback(const NewtonBody* body, const dFloat* matrix, int threadIndex)
...
CLPhysicEntity::CLPhysicEntity(dMatrix Matrix, float masse, dVector Scale, int iNewtonMat, NewtonWorld* world, NewtonCollision* coli) : fMasse(masse), Collision(coli)
{
	...
	if (fMasse) {
		...
		NewtonBodySetForceAndTorqueCallback(pbody, EntityForceAndTorqueCallback);
		NewtonBodySetTransformCallback(pbody, EntityTransformationCallback);
	}
/*
...
CLPhysicEntity::~CLPhysicEntity()
{
	...
	if (pbody)	NewtonDestroyBody(pbody);   pbody = NULL;   ///  !!!!!!!!!!!  a remplacer par DestroyAllBodies()    non?
}
...
NewtonCollision* CLPhysicEntity::BuildPhysicMesh(CTVMesh* Mesh, NewtonWorld* World) {
	...
	NewtonCollision* MeshCollision = NewtonCreateConvexHullFromMesh(World, nMesh, 0.05, 0);
...
NewtonCollision* CLPhysicEntity::BuildPhysicStaticBody(CTVMesh* Mesh, NewtonWorld* World) {
	//  Build the physics mesh and colision tree	
	float Temp;
	int i, vec1, vec2, vec3, Group, color;

	int NbrFaces = Mesh->GetTriangleCount();
	int NbrVertex = Mesh->GetVertexCount();
	char* name = Mesh->GetMeshName();
	std::cout << std::endl << "Creating " << name << " physic body" << "- Faces/Vertex: " << NbrFaces << "/" << NbrVertex << std::endl;
