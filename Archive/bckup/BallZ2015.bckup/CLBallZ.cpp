...
void BallZForceAndTorqueCallback(const NewtonBody* body, dFloat timestep, int threadIndex)
{
	...
	if (cl3DObject->bCTRLBALL)
	{
		...
		dVector finaltorque(Torque.Scale(inertie.x *75.0f) - Torque.Scale(2.0f * inertie.x * (Omega % Torque)));  // !?!?!?  sert a rien finaltorque??
...
CLBallZ::CLBallZ(NewtonWorld* World, cTV_3DVECTOR pos, float Radius, float Masse,int iNewtonMat)
{
	...
	clShell->Mesh->SetAlphaTest(true, 128, true, -1);
...
{
	cTV_3DVECTOR pos = clShell->Mesh->GetPosition();
	clCtrl->SetPosNRot(pos, clCtrl->Mesh->GetRotation());
	clGlobalVar->fBallSpeed = clSpeedo->CalculateSpeed(pos, clGlobalVar->fTimeElapsed);
}
