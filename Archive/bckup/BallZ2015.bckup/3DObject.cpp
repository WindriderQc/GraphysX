...
CL3DObject::~CL3DObject(void)
{
	...
	if (Mesh)  {
		char* name = Mesh->GetMeshName(); // for testing
		Mesh->Destroy();  Mesh = NULL; }
}
void CL3DObject::SetPosNRot(cTV_3DVECTOR Pos, cTV_3DVECTOR Rot)
{
		Mesh->SetPosition(Pos.x,Pos.y,Pos.z);
...
void DebugShowGeoCollision (void* userData, int vertexCount, const dFloat* faceVertec, int id)
{
...
void CL3DObject::NewtonDebug(bool Body, bool Velocity)
{
	if (pbody != NULL)
	{
		dMatrix matrix;
...
void CL3DObject::Translate(cTV_3DVECTOR Force, float SpeedFactor)
{
	...
	if (bAnimState == 1)
	{
		if ((iAnimNbrFrame == -1) || (iAnimNbrFrame >= iActualFrame))  // Si l'anim est infini ou pas terminée
		{
			...
			SetPosNRot(VecPosition, VecRotation);
			iActualFrame++;
		}
		else{
			bAnimState = 0;
