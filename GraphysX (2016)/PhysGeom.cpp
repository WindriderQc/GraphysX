...
namespace GraphysX
{
	...
	//std::vector<CLPhysGeom*> CLPhysGeom::vecItem;
	...
	CLPhysGeom::CLPhysGeom(NewtonWorld* world, STMeshParam stParam, ePHYSMAT physmat, float masse) : CLMeshPrimitive(stParam), m_World(world), physMat(physmat), fMasse(masse)
	{
		pbody = loadBody();

		setPhysmat(physMat);
		
		if (masse != 0) // static object
		{
			matrix_setMass();
	...
	NewtonCollision* CLPhysGeom::loadCollision()
	{
		...
		case eGeometry::CUSTOM_MESH:
			Collision = BuildCustomBody(mesh, !fMasse, m_World);
			break;
		default:
	...
	void CLPhysGeom::matrix_setMass()
	{
		...
		dVector inertie;
		inertie.m_x = 0.7f * fMasse * (stParam.scale.y * stParam.scale.y + stParam.scale.z * stParam.scale.z) / 12;
		inertie.m_y = 0.7f * fMasse * (stParam.scale.x * stParam.scale.x + stParam.scale.z * stParam.scale.z) / 12;
		inertie.m_z = 0.7f * fMasse * (stParam.scale.x * stParam.scale.x + stParam.scale.y * stParam.scale.y) / 12;
