...
CLLand::CLLand(char* HeightmapPath, char* TexturePath, float fWidth, float fHeight, cTV_3DVECTOR Pos)  // Rajouter la pos et le width/Height
{
	...
	pTVLand->GenerateTerrain(HeightmapPath, cTV_PRECISION_BEST, int(fWidth), int(fHeight) ,0,0,0,true);
	pTVLand->SetLightingMode(cTV_LIGHTING_MANAGED);
	
	texGround = pTVTexFactory->LoadTexture(TexturePath, "Ground", -1, -1, cTV_COLORKEY_NO, true);
