...
public:
	...
	void UpdateScene(void)
		{
		 //  SEMBLE PAS UNE FONCTION GÉNÉRAL A TOUTE LES SCENE...  POURRAIT PLANTER SI CALLER AEC LE aCTIVEsCENE DE AREA  
		};
	void ManageCamCtrl(float fWalk, float fStrafe, float fRaise, float fMouseDeltaX, float fMouseDeltaY)
	{
		...
		pTVCamera->ChaseCamera(pCameraAim,new cTV_3DVECTOR(50,5,60),new cTV_3DVECTOR(0,0,0),100,true,-1);
		
		clBallZ->ManageInput();  //  A pas rapport dans une function de manage cam :)
	...
	void InitScene(void)
		{
			...
			// Create BallZ
			clBallZ = new CLBallZ();
			iGroundTex = pTVTexturefactory->LoadTexture("Media\\marble04.jpg", "Ground");
			clTrack = new CL3DObject("Media\\Box.tvm",iGroundTex,cTV_3DVECTOR(0,0,0), cTV_3DVECTOR(50,.5,50),true, 0);
			...
			pTVCamera->SetPosition(-200,600, 400);
			pTVCamera->SetLookAt(0,250,0);
