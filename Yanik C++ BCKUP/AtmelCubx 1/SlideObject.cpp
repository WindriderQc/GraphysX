...
CLSlideObject::CLSlideObject(char* PathToMesh, float Size, cTV_3DVECTOR Pos, bool bEnable)
{
	...
	if(bResult == false)
		MessageBoxA(NULL, "The Slide TVM model hasn't been found...", "Warning", MB_ICONEXCLAMATION|MB_SETFOREGROUND);
	Mesh->SetPosition(Pos.x, Pos.y, Pos.z);//-500,500,0); 
	Mesh->SetScale(Size,Size,Size); // 12
	...
	iPhysicBody = pTVPhysics->CreateStaticMeshBody(Mesh, false, true); 
    
//	pTVPhysics->SetDamping(iPhysicBody,0.00001, new cTV_3DVECTOR(0.00001,0.00001,0.00001));

}

CLSlideObject::~CLSlideObject(void)
{
	delete(pTVScene); pTVScene = NULL;
	delete(pTVPhysics); pTVPhysics = NULL;
}

void CLSlideObject::SetPosition(cTV_3DVECTOR position)
{
	Mesh->SetPosition(position.x, position.y, position.z);
}

cTV_3DVECTOR CLSlideObject::GetPosition(void)
{
	return(Mesh->GetPosition());
}
CTVMesh* CLSlideObject::GetMesh(void)
{
return(Mesh);
}

int CLSlideObject::GetPhysicBody(void)
{
return(iPhysicBody);
}
void CLSlideObject::Render(void)
{
	if(Mesh->IsEnabled())
		Mesh->Render();
}