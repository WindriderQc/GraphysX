...
//  Mauvaise technique...  ca va prendre une facon de UnloadScene car sinon en changeant d'une scene a l'autre...   la mem va craquer...
	...
	case NO_SCENE:
	default:
			clDefaultScene = new CLScene("", "", "Media\\Sky\\Winter\\", "media\\sky\\clearnight\\");
			break;
...
void CLArea::RenderArea(float TimeElapsed, long lTimeOfDay)
{
...
case CAR_SCENE:
	//Render Objects
	clCarScene->Render(TimeElapsed, lTimeOfDay);
...
default:
	clDefaultScene->Render(TimeElapsed, lTimeOfDay);
	break;
}
...
if(clGlobalVar->GetRenderAtmos())
	{
		sprintf(BufferTime, "%d", int(clCarScene->GetSceneTimeOfDay()));  //  ptete enlever le int() pour mettre /1000  a la place
