...
long CLScene::GetSceneTimeOfDay(void)
{
return (clAtmos->lTimeOfDay);
}

void CLScene::Render(float TimeElapsed, long lTimeOfDay)
{
	//Render Atmosphere
	if(clAtmos != NULL)
		clAtmos->UpdateAndRender(TimeElapsed, lTimeOfDay); // A arranger car l'esu ne sera pas renderer..  manque le render sur les surface...
