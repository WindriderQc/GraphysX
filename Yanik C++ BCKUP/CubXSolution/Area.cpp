...
{
	if((pActiveScene != NULL) && (pActiveScene != clDefaultScene))
	  {
	  if(pActiveScene == clSkySelectScene)
		  clDefaultScene->SetSkybox(clSkySelectScene->pActiveSkybox);  // Puisque la scene par défault n'est jamais supprimer, elle conserveras le pointer de la Skybox selectionnée.
	...
	case SKYBOX_SELECT_SCENE:
			...
			break;
	case CAR_SELECT_SCENE:
			clCarSelectScene = new CLCarSelectScene();
			clCarSelectScene->InitScene();
			pActiveScene = clCarSelectScene;
			break;
...
{
long lTime = clClock->getTimeOfDay();
//  Render Water surfaces
pActiveScene->UpdateWater(lTime);

clTV3D->StartRenderingBlock();
pActiveScene->Render(lTime);
pActiveScene->UpdateScene();
...
if(clGlobalVar->GetbUSE_TV_PHYSICS())
		pTVPhysics->Simulate(fTimeElapsed * 0.0025f);

pActiveScene->ManageCamCtrl(fWalk, fStrafe, fRaise, fMouseDeltaX, fMouseDeltaY, fTimeElapsed);
}
