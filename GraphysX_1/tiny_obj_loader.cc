...
bool LoadObj(std::vector<shape_t> &shapes, // [output]
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'v' && isSpace((token[1]))) {
	  ...
	  pos.y = y;
	  pos.z = z;
	  shape.VerticesPos.push_back(pos);
      continue;
    }
    // normal
  ...
  bool ret = exportFaceGroupToShape(shape, vertexCache, v, vn, vt, faceGroup,
                                    material, name, true);
  //if (ret) {  // ptit hack car ca retourne false sur un spline car il n'y a pas de face.
  // A ARRANGER POUR MIEUX UTILISER LA CLASSE ET UTILISER LE VerticesINDEX
